...
//------------ST7735_DrawString2------------
// String draw function with characters of double size in landscape mode.
// place beginning of string at any x,y position (where it will fit)
// Input: x pixel location of start of string x coordinate
// y pixel location of start of string x coordinate
// pt pointer to a null terminated string to be printed
// textColor 16-bit color of the characters
// bgColor is set to same color as text to overwrite the background screen and
// size is 2
// Output: number of characters printed that will fit to screen
uint32_t ST7735_DrawString2(uint16_t x, uint16_t y, char *pt, int16_t textColor, int16_t bgtext){
    uint32_t count = 0;
    if(y>127-16) return 0;
    while(*pt){
        ST7735_DrawCharS(x+count*10, y, *pt, textColor, bgtext, 2);
        pt++;
        x = x+1;
        if(x>159-10) return count; // number of characters printed
        count++;
    }
    return count; // number of characters printed
}
